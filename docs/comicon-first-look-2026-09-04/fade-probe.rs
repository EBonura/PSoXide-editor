...
impl Probe {
    fn release_for_data_reads(&mut self, _: u32) {
        self.stopped = true;
    }
...
fn main() {
    for (start, target, duration) in [(0, 80, 60), (80, 0, 120), (0, 40, 60), (40, 0, 120)] {
        let mut p = Probe {
            current_volume_percent: start,
            target_volume_percent: target,
            fade_ticks_left: duration,
            fade_out_stop: target == 0,
            routed: false,
            stopped: false,
        };
        let mut arrived = 0;
        for tick in 1..=duration + 1 {
            p.tick_fade(tick as u32);
            if p.current_volume_percent == target && arrived == 0 {
                arrived = tick;
            }
        }
        println!(
            "{} -> {}: requested {} ticks, reached target after {} ticks",
            start, target, duration, arrived
        );
    }
}
