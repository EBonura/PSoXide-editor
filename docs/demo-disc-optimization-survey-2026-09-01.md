...
| B2 `fix/cook-uv-wrap` | on main (`be93eb6b`), Manny approved the pair | +6.10% | differ (intended) | 371 surfaces split, +693 faces; tape-end pair: guardrail tiles instead of smearing; needs B1 to link |
...
| B4/B5 `perf/i32-kernels` (`edbc3b96`) | pushed for review | -0.31% cycles, -2.45% instructions vs main `891850b6` | display identical at controller poll 5,250 (`--stop-at-poll`), world pixels identical at tape end | FrustumPlanes in i32 (near exact and bit-identical, side planes rounded with an undecided band under 0.1 px), Q16 clip fractions exact in u32, mover/sky/asset divisions in u32; 154 psx-bsp tests incl. 290k-point oracle sweep. Tape-end frames alone are NOT byte-comparable: runs can stop one poll apart, fixed by `--stop-at-poll` (`531c2806`). A reused guest stage root served a stale exe once; the bench now clears it |
| B6 `perf/idle-motor-probes` | on main (`891850b6`) | -0.10% vs main `5d2e45ae` | identical | memoise `player_contents` per (position, height); only idle ticks benefit, the tape is mostly in motion |
