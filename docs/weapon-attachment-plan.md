...
**P4. Joint names** (small, big UX payoff) -- DONE 2026-08-10
The cooker now carries source bone names (post-collapse, cooked-joint order) out through `RigidModelPackage.joint_names`; imports backfill them onto the (signature-deduped) `SkeletonResource`; the socket list, socket picker, capsule Body-joint row, and pose-key picker show "13 · RightHand" labels with rig prefixes (`mixamorig:`) stripped for display. The `backfill_joint_names` example re-cooks Model sources for names only; running it on cortex_anim named the shared 22-bone rig from idle.glb (the Rust Mantis and CI Player FBX sources are missing on disk, but they share that skeleton, so everything is covered). Immediate payoff: the P1 provisional socket sat on joint 9 = `mixamorig:LeftHand`; the real right hand is joint 13, and the project sockets now point there. Old unnamed skeletons keep working; the compatibility signature is untouched.

**P5. Enemy equipment rendering** (small to medium) -- DONE 2026-08-10
Close G5, shipped as: (a) an additive cooked field `EquipmentRecord.model_instance` (sentinel `NO_INSTANCE` for the player) binding each non-player equipment record to its host entity's model instance (the cook reuses the last-pushed-instance convention game entities already use); (b) `instance_pose_context`, extracted from the instance draw loop as its single source of truth for live position/yaw/state-clip/phase composition, consumed by both the loop and the new pass; (c) `draw_instance_equipment`, a room-gated per-room pass that composes each bound weapon from the instance's LIVE pose (socket pose from the same context, no crossfade, matching the body) and submits through `submit_equipped_weapon`, which is now shared with the player pass. Gate met headless: the Rust Mantis patrols with the heavy sword riding its walk animation.
Notes from the gate hunt: the cortex_anim sandbox's "Mantis Enemy" node shipped with `rotation_degrees: (-90, 4, 0)`, cooking `pitch: 3072` and lying the enemy INTO the floor, which is why it never appeared in any prior dump (pre-existing scene artifact, now zeroed); the enemy AI's flanking also deliberately circles BEHIND the player (the camera's blind side), so the gate places it out of aggro on the visible side where its patrol crosses the camera. The mantis spawn in cortex_anim was moved in front of the player and stood upright for authoring visibility.
