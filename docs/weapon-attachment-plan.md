...
**P2. Socket mode in the animation view** (the core ask, medium)
Third authoring mode beside Combat Volumes and Pose Keys: `show_attachment_sockets` plus selection on `ModelAnimationViewerState` (`model_animation_viewer.rs:69`), included in the `joint_picking` / panel predicates (`:563`). New `draw_attachment_socket_editor` panel mirroring `draw_combat_capsule_editor` (`:1847`), embedding the existing `attachment_socket_list_editor` and the Move/Rotate drag pattern from `manipulate_selected_capsule` (`:2218`). Click a joint in the preview to set `socket.joint` (analogue of `attach_selected_capsule_to_joint`, `:2187`). Preview draws an axis-triad marker at the composed socket pose via `joint_local_point` (`model_import_preview.rs:1168`). Tests: harness-driven pick test in psxed-ui plus a pure-render marker position test.

**P3. Equipped-weapon preview** (medium)
`ImportPreviewOptions` grows an optional equipped-weapon overlay (weapon model bytes, atlas, grip); render it after the character using the runtime's own composition (`compose_socket_pose` then grip inverse, four lines of math, `equipment.rs:123`). Toolbar picker chooses a WeaponResource. Gate: scrub light/heavy attack clips with the sword tracking the hand through the crossfade.
