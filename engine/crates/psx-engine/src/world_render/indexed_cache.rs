...
>(
    ...
    screen_margin: i32,
    mut prebuilt_pool: Option<(&mut [QuadTexturedGouraud], bool)>,
    ...
    for accepted_index in 0..accepted_cell_count {
        ...
        let mut i = first;
        while i < end {
            // Per-surface prebuilt pool slot: indexed by the surface's
            // position in the room's surface slice; out-of-pool
            // surfaces fall back to the per-frame arena path.
            let surface_prebuilt = match prebuilt_pool.as_mut() {
                Some((pool, fill)) => pool.get_mut(i).map(|quad| (quad, *fill)),
                None => None,
            };
