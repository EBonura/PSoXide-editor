...
/// Divide `numerator / denominator` into raw signed Q20.12 using only
/// 32-bit arithmetic.
///
/// This avoids the software 64-bit division helpers generated by the PS1
/// target for `(numerator as i64 * 4096) / denominator` while still keeping
/// the full 12 fractional bits. A zero denominator returns zero.
#[inline]
pub fn div_q12_i32(numerator: i32, denominator: i32) -> i32 {
    if denominator == 0 {
        return 0;
    }
    let negative = (numerator < 0) ^ (denominator < 0);
    let numerator = abs_i32_u32(numerator);
    let denominator = abs_i32_u32(denominator);
    let whole = numerator / denominator;
    let max_whole = (i32::MAX as u32) >> Q12::FRACTIONAL_BITS;
    if whole > max_whole {
        return if negative { i32::MIN } else { i32::MAX };
    }

    let fraction =
        div_q12_fraction(numerator % denominator, denominator) & ((Q12::SCALE as u32) - 1);

    let magnitude = (whole << Q12::FRACTIONAL_BITS) | fraction;
    if magnitude > i32::MAX as u32 {
        ...
        (magnitude as i32).saturating_neg()
    } else {
        magnitude as i32
    }
}
...
#[inline(always)]
fn div_q12_fraction(remainder: u32, denominator: u32) -> u32 {
    #[cfg(target_arch = "mips")]
    {
        ...
        unsafe {
            core::arch::asm!(
                ".set push",
                ".set noreorder",
                // The quotient is known to be below one, so twelve binary
                // long-division steps produce its complete Q12 fraction. Put
                // the next remainder shift in the loop branch's delay slot;
                // the harmless thirteenth shift is discarded on exit.
                "ori $10, $0, 12",
                "sll $8, $8, 1",
                "2:",
                "sll $9, $9, 1",
                ...
                "sll $8, $8, 1",
                ".set pop",
                inlateout("$8") remainder => _,
                inlateout("$9") fraction,
                out("$10") _,
                out("$11") _,
                in("$12") denominator,
    ...
    #[cfg(not(target_arch = "mips"))]
    {
        ...
        let mut fraction = 0u32;
        let mut bit = (Q12::SCALE as u32) >> 1;
        while bit != 0 {
            remainder <<= 1;
...
mod tests {
    ...
    #[test]
    fn q12_division_uses_32_bit_fractional_steps() {
        assert_eq!(div_q12_i32(1, 2), Q12::HALF.raw());
        assert_eq!(div_q12_i32(-3, 2), -6144);
        assert_eq!(div_q12_i32(0x10_0000, 0x20_0000), Q12::HALF.raw());
        assert_eq!(div_q12_i32(7, 3), 9557);
        assert_eq!(div_q12_i32(1, 0), 0);
    }

    #[test]
    fn q12_division_matches_signed_i64_reference_in_representable_range() {
        const NUMERATORS: [i32; 13] = [
            i32::MIN,
            -0x10_0000,
            -8193,
            -4096,
        ...
        for numerator in NUMERATORS {
            for denominator in DENOMINATORS {
                let reference = ((numerator as i64) << Q12::FRACTIONAL_BITS) / denominator as i64;
                if (i32::MIN as i64..=i32::MAX as i64).contains(&reference) {
                    assert_eq!(
                        div_q12_i32(numerator, denominator),
                        reference as i32,
