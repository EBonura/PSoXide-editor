//! PA5: isolate stale BIOS reverb state from the Hazard Course map-bank DMA.
//!
//! PA4 SPLIT proved that the persistent hardware-only noise starts when the
//! t0a0 map bank is uploaded, while every voice mixer volume is zero.  PA5
//! preserves the exact full -> init -> light -> map sequence, but inserts a
//! selectable reverb-state reset immediately before the map upload.  The QR
//! includes the untouched pre-engine boot snapshot and all 32 raw BIOS reverb
//! configuration words so the silicon state can be replayed in PSoXide.
...
const STAGE_FRAMES: [u16; STAGE_COUNT] = [180, 180, 45, 180, 180, 180, 180, 180];
const CAPTURE_FRAMES: [u16; STAGE_COUNT] = [120, 120, 44, 120, 120, 120, 120, 120];
...
const EON_HI: u32 = 0x1F80_1D9A;
const REVERB_BASE: u32 = 0x1F80_1DA2;
const EXT_VOL_L: u32 = 0x1F80_1DB4;
const EXT_VOL_R: u32 = 0x1F80_1DB6;
...
// 424 bytes -> 568 Base64 chars.  Version 20-M fits the complete payload
// while remaining 210 pixels square including the four-module quiet zone.
...
#[derive(Copy, Clone)]
pub(crate) struct ReverbSnapshot {
    spucnt: u16,
    spustat: u16,
    reverb_vol_l: u16,
    reverb_vol_r: u16,
...
impl ReverbSnapshot {
    ...
    pub(crate) fn capture() -> Self {
        ...
        let mut i = 0usize;
        while i < cfg.len() {
            cfg[i] = read(REVERB_CFG_BASE + i as u32 * 2);
            i += 1;
        }
        Self {
            ...
            ext_vol_l: read(EXT_VOL_L),
            ext_vol_r: read(EXT_VOL_R),
            eon_lo: read(EON_LO),
            eon_hi: read(EON_HI),
            cfg,
        }
    ...
    fn cfg_hash(self) -> u32 {
        fnv16_values(&self.cfg)
    }

    fn cfg_nonzero(self) -> u16 {
        self.cfg.iter().filter(|&&value| value != 0).count() as u16
    }
}
...
impl Variant {
    ...
    const fn label(self) -> &'static str {
        match self {
            Self::Control => "CONTROL - KEEP STALE REVERB",
            Self::Depth0 => "DEPTH0 - ZERO WET VOLUME",
            Self::Depth2 => "DEPTH2 - ZERO WET + WAIT 2",
            Self::Base0 => "BASE0 - ZERO REVERB BASE",
            Self::Full0 => "FULL0 - CLEAR ALL REVERB STATE",
        }
    }
...
impl ReverbProbe {
    pub(crate) const fn new(boot: ReverbSnapshot) -> Self {
        Self {
            boot,
            selecting: true,
            selection_frame: 0,
            variant: Variant::Depth0,
            stage: 0,
            stage_frame: 0,
            complete: false,
            run: 0,
    ...
    pub(crate) fn start(&mut self) {
        ...
        self.variant = Variant::Depth0;
        self.complete = false;
        self.qr_size = 0;
        spu::init();
        spu::enable_cd_audio(false);
        tty::println("hardware-tests: pa5 selection; default DEPTH0; reboot per variant");
    }
    ...
    fn begin_run(&mut self) {
        ...
        spu::enable_cd_audio(false);
        self.apply_stage();
        tty::print("hardware-tests: pa5 begin variant=");
    ...
    fn apply_stage(&mut self) -> bool {
        let blocking = match self.stage {
            ...
            5 => {
                self.timed_event(|this| this.apply_reverb_variant());
                self.variant.wait_vblanks() != 0
            }
            6 => {
        ...
        tty::print("hardware-tests: pa5 stage=");
    ...
    fn apply_reverb_variant(&mut self) {
        match self.variant {
            Variant::Control => {}
            Variant::Depth0 | Variant::Depth2 => {
                write16(REVERB_VOL_L, 0);
                write16(REVERB_VOL_R, 0);
                for _ in 0..self.variant.wait_vblanks() {
                    interrupts::wait_vblank();
                }
            }
            Variant::Base0 => write16(REVERB_BASE, 0),
            Variant::Full0 => {
                // Mute the wet output first, then clear every producer and
                // work-area register.  The main dry output remains enabled.
                write16(REVERB_VOL_L, 0);
                write16(REVERB_VOL_R, 0);
                write16(EON_LO, 0);
                write16(EON_HI, 0);
    ...
    fn load_full(&mut self) {
        ...
        assert_eq!(end - SPU_SAMPLE_BASE, PA3_FULL_BYTES, "PA5 full bank size");
    }
    ...
    fn upload_map(&mut self) {
        assert!(self.map_base != 0, "PA5 map base not established");
        let end = self.upload_layout(PA3_MAP_LAYOUT, Pattern::MapFixture, self.map_base);
        assert_eq!(end - self.map_base, PA3_MAP_BYTES, "PA5 map bank size");
        assert!(end <= 512 * 1024, "PA5 fixture exceeds SPU RAM");
    }
    ...
    fn play_menu(&self) {
        assert!(self.full_menu_addr != 0, "PA5 menu sample not loaded");
    ...
    fn readback_map(&mut self) {
        assert!(self.map_base != 0, "PA5 map bank not loaded");
        self.observed_hash = stable_read_hash(self.map_base);
    }

    fn capture_stage(&mut self, tick: u32) {
        let stage = self.stage as usize;
        let snapshot = if stage == 0 {
            self.boot
        } else {
            ReverbSnapshot::capture()
        };
        ...
        self.records[stage].fields = [
            ...
            ((snapshot.spucnt as u32) << 16) | snapshot.spustat as u32,
            ((snapshot.reverb_vol_l as u32) << 16) | snapshot.reverb_vol_r as u32,
            ((snapshot.reverb_base as u32) << 16) | snapshot.ext_vol_l as u32,
            ((snapshot.ext_vol_r as u32) << 16) | snapshot.eon_lo as u32,
            ((snapshot.eon_hi as u32) << 16) | snapshot.cfg_nonzero() as u32,
            snapshot.cfg_hash(),
            event_vblanks,
            detail_a,
            detail_b,
        ];
        tty::print("hardware-tests: pa5 sample stage=");
        tty::print(hex2(self.stage).as_str());
        tty::print(" rvol=");
        tty::print(hex8(self.records[stage].fields[2]).digits());
        tty::print(" base=");
        tty::print(hex8(snapshot.reverb_base as u32).digits());
        tty::print(" cfg=");
        tty::println(hex8(snapshot.cfg_hash()).digits());
    }
    ...
    fn encode_qr(&mut self) {
        ...
        append(&mut text, &mut text_len, b"PA5/");
    ...
    fn write_binary(&self, out: &mut BinaryBuffer<'_>, crc: u32) {
        ...
        for value in self.boot.cfg {
            out.push_u16(value);
        }
        out.push_u32(crc);
        assert_eq!(out.len(), BINARY_LEN, "PA5 binary layout drift");
    }
    ...
    fn print_payload(&self) {
        ...
        let mut payload = [0u8; BASE64_LEN];
        base64_encode(&binary, &mut payload);
        tty::print("hardware-tests: pa5 PA5/");
    ...
    pub(crate) fn draw(&self, font: &FontAtlas) {
        font.draw_text(8, 8, "HL REVERB STATE PA5", (255, 232, 128));
        if self.selecting {
            font.draw_text(
                8,
                24,
            ...
            font.draw_text(8, 102, "RUN DEPTH0 FIRST", (96, 240, 128));
            font.draw_text(8, 118, "THEN REBOOT: CTRL, FULL0", (150, 170, 200));
            font.draw_text(8, 134, "BASE0 / DEPTH2 ARE DIAGNOSTIC", (255, 128, 96));
            let remaining = SELECTION_FRAMES.saturating_sub(self.selection_frame);
            font.draw_text(8, 168, "AUTO DEPTH0", (140, 160, 190));
        ...
        if !self.complete {
            ...
            font.draw_text(
                8,
                108,
                "NOTE NOISE ONSET RELATIVE TO STAGE 06",
                (112, 136, 170),
            );
            ...
            font.draw_text(
                8,
                140,
                "QR INCLUDES ALL 32 BIOS REVERB WORDS",
                (112, 136, 170),
            );
            return;
        }

        font.draw_text(8, 28, "COMPLETE - HOLD CAMERA STEADY ON QR", (96, 240, 128));
        font.draw_text(8, 230, "X SELECT AGAIN  DOWN PA4", (150, 170, 200));
    ...
    fn stage_label(&self) -> &'static str {
        match self.stage {
            0 => "BIOS SNAPSHOT + CALIBRATION",
            1 => "FULL MENU BANK",
            2 => "V16 NATURAL END",
            3 => "SPU INIT ONLY",
            4 => "LIGHT BANK ONLY",
            5 => "REVERB RESET ONLY",
            6 => "T0A0 MAP BANK ONLY",
            _ => "SPU READBACK ONLY",
        }
    }
    ...
    fn stage_description(&self) -> &'static str {
        match self.stage {
            0 => "PRE-ENGINE BIOS REGS SAVED; SHORT BEEP",
            1 => "EXACT CHUNK 3000 PRODUCTION DMAS",
            2 => "MENU V16 ENDS NATURALLY",
            3 => "SDK INIT; NO BANK UPLOAD",
            4 => "UPLOAD CHUNK 3050; THEN OBSERVE",
            5 => self.variant.label(),
            6 => "UPLOAD CHUNK 3198; NO VOICES ACTIVE",
            _ => "64-BYTE STABLE READ; HASH IN QR",
        }
    }

    fn stage_color(&self) -> (u8, u8, u8) {
        if self.stage == 5 && self.variant != Variant::Control {
            (96, 240, 128)
        } else if self.stage == 6 && self.variant == Variant::Control {
            (255, 96, 96)
        } else {
            (255, 216, 96)
        }
