...
pub(super) fn average4_i32(a: i32, b: i32, c: i32, d: i32) -> i32 {
    a.saturating_add(b).saturating_add(c).saturating_add(d) / 4
}

pub(super) fn abs_delta_i32(a: i32, b: i32) -> i32 {
    if a >= b {
        a.saturating_sub(b)
    } else {
        b.saturating_sub(a)
...
pub(super) fn rotate_x_q12(v: [i32; 3], angle_q12: u16) -> [i32; 3] {
...
pub(super) fn rotate_y_q12(v: [i32; 3], angle_q12: u16) -> [i32; 3] {
    let angle = Angle::from_q12(angle_q12);
    let s = angle.sin().raw();
    let c = angle.cos().raw();
    [
...
pub(super) fn rotate_z_q12(v: [i32; 3], angle_q12: u16) -> [i32; 3] {
    let angle = Angle::from_q12(angle_q12);
    let s = angle.sin().raw();
    let c = angle.cos().raw();
    [
