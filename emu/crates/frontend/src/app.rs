...
impl AppState {
    ...
    ///    `absolute_bin_path → (cue_title, cue_path)` so each BIN
    ///    the CUE owns shows up with the CUE's friendly filename
    ///    as its title (e.g. the disc's own name instead of
    ///    the raw PVD ID "SCUS-94900"). Launch the exact CUE path;
    ///    its library entry retains the stable save-state identity.
    /// 2. Walk every entry. SDK examples and project builds launch
    ///    from their CUEs; their EXE/BIN siblings are intermediates.
    ///    Retail BIN entries still use a CUE title/path when one owns
    ...
    /// title, and clicking it launches that exact CUE or standalone image.
    pub fn refresh_menu_library(&mut self) {
        use std::collections::{HashMap, HashSet};

        let sdk_examples_root = self.resolve_sdk_examples_dir().filter(|p| p.exists());

        // Pass 1: map "BIN path" → (CUE-derived title, exact CUE path).
        ...
        for e in &self.library.entries {
            ...
            match e.kind {
                ...
                GameKind::DiscBin | GameKind::DiscIso | GameKind::DiscCcd => {
                    ...
                    if let Some(item) = game_menu_item(e, &cue_owns_bin, &mut cue_already_listed) {
                        games.push(item);
                    }
                }
                GameKind::Exe => {
...
/// Load an authored disc through the built-in runtime.
...
mod tests {
    ...
    fn games_rows_with_shared_disc_ids_launch_the_selected_file() {
        ...
        let rows: Vec<_> = [0, 2, 4]
            .into_iter()
            .map(|i| game_menu_item(&entries[i], &owners, &mut listed).unwrap())
            .collect();
        assert_eq!(
            rows.iter().map(|r| r.title.as_str()).collect::<Vec<_>>(),
            ["Normal", "Telemetry", "Standalone"]
        );
        assert!(game_menu_item(&entries[0], &owners, &mut listed).is_none());
