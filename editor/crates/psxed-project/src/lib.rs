...
/// Runtime panorama texture width, in 8bpp texels.
pub const SKY_PANORAMA_WIDTH: u16 = 256;
/// Runtime panorama texture height, in 8bpp texels.
pub const SKY_PANORAMA_HEIGHT: u16 = 128;
const SKY_PANORAMA_PALETTE_COLORS: usize = 256;
...
/// Bake the resolved cyclorama into a single 8bpp PSXT panorama.
///
...
/// this texture path so the per-frame sky cost is just a couple of
/// textured screen quads instead of hundreds of projected backdrop
/// polygons.
pub fn generate_sky_panorama_psxt(sky: ResolvedSkySettings) -> Option<Vec<u8>> {
    ...
    let (palette, indices) = sky_quantize_panorama(&pixels);
...
fn rasterize_sky_cyclorama_quad(
    ...
    let mut vertices = [
        sky_raster_vertex(quad.direction_q12[0], quad.rgb[0], width, top_pitch, bottom_pitch),
        sky_raster_vertex(quad.direction_q12[1], quad.rgb[1], width, top_pitch, bottom_pitch),
        sky_raster_vertex(quad.direction_q12[2], quad.rgb[2], width, top_pitch, bottom_pitch),
        sky_raster_vertex(quad.direction_q12[3], quad.rgb[3], width, top_pitch, bottom_pitch),
    ];
...
fn sky_raster_vertex(
    ...
    let v = ((top_pitch - pitch) / (top_pitch - bottom_pitch).max(0.001))
        * SKY_PANORAMA_HEIGHT as f32;
...
fn sky_quantize_panorama(pixels: &[[u8; 3]]) -> (Vec<[u8; 3]>, Vec<u8>) {
    ...
    while boxes.len() < SKY_PANORAMA_PALETTE_COLORS {
    ...
    palette.truncate(SKY_PANORAMA_PALETTE_COLORS);
    let indices = pixels
        .iter()
...
fn push_mountain_cyclorama(
    ...
    let mountain_columns = (columns * 3).clamp(24, SKY_CYCLORAMA_MOUNTAIN_COLUMNS_MAX);
