...
pub struct SkySettings {
    ...
    /// Gap between the horizon and the mountain ridge. Lower values
    /// keep the mountains closer to the cloud/horizon band.
...
const SKY_CYCLORAMA_CLOUD_STREAK_MAX: usize = 10;
const SKY_CYCLORAMA_CLOUD_HERO_STREAKS: usize = 4;
const SKY_CYCLORAMA_CLOUD_SEGMENTS_MAX: usize = 16;
...
const SKY_CYCLORAMA_SUN_GLOW_RINGS: usize = 5;
const SKY_CYCLORAMA_SUN_BORDER_RINGS: usize = 3;
const SKY_CYCLORAMA_SUN_CORE_RINGS: usize = 2;
const SKY_CYCLORAMA_SUN_QUAD_MAX: usize = SKY_CYCLORAMA_SUN_SEGMENTS
    * (SKY_CYCLORAMA_SUN_GLOW_RINGS
        + SKY_CYCLORAMA_SUN_BORDER_RINGS
        + SKY_CYCLORAMA_SUN_CORE_RINGS);
...
fn push_sun_cyclorama(
    ...
    if glow_t > 0.0 && glow_size_t > 0.0 {
        push_sun_polar_rings(
            ...
            |sky, point_yaw, point_pitch, radius_t, theta| {
                let falloff = (1.0 - radius_t.clamp(0.0, 1.0)).powf(1.45);
                let alpha = (36.0 + glow_t * 142.0) * falloff;
                let highlight = sun_directional_weight(theta, 0.68, 2.2);
                let tint = cyclorama_lerp_rgb(
                    brighten_rgb(sky.sun_border_color, 18),
                    [255, 206, 156],
                    (highlight * 112.0).clamp(0.0, 255.0) as u8,
                );
    ...
    push_sun_polar_rings(
        ...
        |sky, point_yaw, point_pitch, radius_t, theta| {
            ...
            let alpha = (154.0 + glow_t * 74.0) * ridge.max(outer_feather * 0.35);
            ...
            let mut tint = cyclorama_lerp_rgb(
                ...
                [255, 226, 184],
                (highlight * 132.0).clamp(0.0, 255.0) as u8,
            );
            tint = cyclorama_lerp_rgb(
                tint,
                [60, 22, 26],
                (shade * 42.0).clamp(0.0, 255.0) as u8,
            );
            sun_tinted_sky_color(
                sky,
                point_yaw,
    ...
    F: FnMut(ResolvedSkySettings, f32, f32, f32, f32) -> [u8; 3],
{
    ...
    for ring in 0..rings {
        let r0 = lerp_f32(inner_radius, outer_radius, ring as f32 / rings as f32);
        let r1 = lerp_f32(
            inner_radius,
            outer_radius,
            (ring + 1) as f32 / rings as f32,
        );
        for segment in 0..SKY_CYCLORAMA_SUN_SEGMENTS {
            let theta0 =
                std::f32::consts::TAU * segment as f32 / SKY_CYCLORAMA_SUN_SEGMENTS as f32;
            let theta1 =
                std::f32::consts::TAU * (segment + 1) as f32 / SKY_CYCLORAMA_SUN_SEGMENTS as f32;
            let (yaw00, pitch00) = sun_polar_point(
                center_yaw,
                center_pitch,
                yaw_radius,
                pitch_radius,
            ...
            let (yaw10, pitch10) = sun_polar_point(
                center_yaw,
                center_pitch,
                yaw_radius,
                pitch_radius,
            ...
            let (yaw01, pitch01) = sun_polar_point(
                center_yaw,
                center_pitch,
                yaw_radius,
                pitch_radius,
            ...
            let (yaw11, pitch11) = sun_polar_point(
                center_yaw,
                center_pitch,
                yaw_radius,
                pitch_radius,
            ...
            let rgb = [
                shade_vertex(sky, yaw00, pitch00, r0, theta0),
                shade_vertex(sky, yaw10, pitch10, r0, theta1),
                shade_vertex(sky, yaw01, pitch01, r1, theta0),
                shade_vertex(sky, yaw11, pitch11, r1, theta1),
            ];
            push_sky_cyclorama_quad_corners(
                out, yaw00, yaw10, pitch00, pitch10, pitch01, pitch11, rgb,
            );
        }
    }
}
...
fn sun_directional_weight(theta: f32, direction: f32, power: f32) -> f32 {
    theta.cos().mul_add(direction.cos(), theta.sin() * direction.sin())
        .max(0.0)
        .powf(power.max(0.01))
}
...
fn sun_tinted_sky_color(
    ...
    let base = sky_color_for_pitch_yaw_core(sky, pitch, yaw, horizon_pitch, top_pitch, bottom_pitch);
...
fn push_star_cyclorama(
    ...
    let count =
        (18.0 + darkness * 34.0 + (1.0 - density_t) * 12.0).round() as usize;
...
fn push_cloud_streak_cyclorama(
    ...
    let segment_count = (9 + usize::from(tile_count / 3) + usize::from(cloud.density / 96))
        .clamp(8, SKY_CYCLORAMA_CLOUD_SEGMENTS_MAX);
    let count = (4 + usize::from(cloud.density / 42) + usize::from(tile_count / 6))
...
fn push_cloud_streak_segments(
    ...
    for segment in 0..segment_count {
        ...
        let pitch0 = center_pitch
            + slant * (t0 - 0.5)
            + cloud_lobe_pitch(seed ^ 0x9e37_79b9, t0, thickness);
        let pitch1 = center_pitch
            + slant * (t1 - 0.5)
            + cloud_lobe_pitch(seed ^ 0x9e37_79b9, t1, thickness);
...
mod tests {
    ...
    fn sky_settings_resolve_clamps_subdivision_defaults() {
        ...
        assert_eq!(
            default_sky.sun_border_color,
            default_sky_sun_border_color()
        );
        ...
        assert_eq!(default_sky.mountain_top_color, default_sky_mountain_top_color());
        assert_eq!(
    ...
    fn sky_cyclorama_sun_uses_faceted_polar_geometry() {
        ...
        assert!(sun_quads.iter().any(|quad| {
            quad.direction_q12[0] == quad.direction_q12[1]
