...
fn push_cloud_streak_cyclorama(
    ...
    let segment_count = (3 + usize::from(tile_count / 4) + usize::from(cloud.density / 96))
        .clamp(3, SKY_CYCLORAMA_CLOUD_SEGMENTS_MAX);
    let count = (2 + usize::from(cloud.density / 72) + usize::from(tile_count / 8))
    ...
    for (bank, offset) in [-28.0_f32, 24.0].iter().enumerate() {
        let width = (68.0 + bank as f32 * 28.0) * width_scale.min(1.2);
        let center_pitch = band_center + (bank as f32 - 1.0) * pitch_spread * 0.18;
        let thickness = 1.1 + extent_t * (2.4 + bank as f32 * 0.55);
        let slant = -2.8 + bank as f32 * 2.6;
        let tint = cyclorama_lerp_rgb(
            cloud.color,
            [255, 166, 150],
            (68.0 + bank as f32 * 24.0) as u8,
        );
...
fn push_cloud_streak_segments(
    ...
    for segment in 0..segment_count {
        let t = ((segment as f32 + 0.5) / segment_count as f32
            + (sky_hash_unit(seed ^ 0xa511_e9b3, segment as u32) - 0.5)
                * (0.46 / segment_count as f32))
            .clamp(0.04, 0.96);
        let lobe_weight = cloud_streak_fade(t)
            * cloud_lobe_weight(seed ^ 0x1b56_c4e9, t)
            * density_t
            * alpha_scale;
        if lobe_weight <= 0.025 {
            continue;
        }
        let stride = width / segment_count as f32;
        let width_jitter = 0.62 + sky_hash_unit(seed ^ 0xc2b2_ae35, segment as u32) * 0.82;
        let half_width = (stride * width_jitter).clamp(2.4, width * 0.32);
        let pitch = center_pitch
            + slant * (t - 0.5)
            + cloud_lobe_pitch(seed ^ 0x9e37_79b9, t, thickness)
            + (sky_hash_unit(seed ^ 0x85eb_ca6b, segment as u32) - 0.5) * thickness * 0.9;
        let local_thickness = thickness
            * (0.78 + sky_hash_unit(seed ^ 0x27d4_eb2d, segment as u32) * 1.12)
            * (0.82 + density_t * 0.42);
        let local_slant =
            slant * (0.46 + sky_hash_unit(seed ^ 0x1656_67b1, segment as u32) * 0.48)
                + (sky_hash_unit(seed ^ 0xd3a2_646c, segment as u32) - 0.5) * 2.4;
        push_cloud_lens_cyclorama(
            out,
            sky,
            yaw_start + width * t,
            half_width,
            pitch,
            local_thickness * 0.96,
            local_slant,
            body,
            lobe_weight * 172.0,
            horizon_pitch,
        ...
        push_cloud_lens_cyclorama(
            out,
            sky,
            yaw_start + width * (t + 0.018),
            half_width * 0.58,
            pitch + local_thickness * 0.16,
            local_thickness * 0.26,
            local_slant * 0.7,
            warm,
            lobe_weight * 250.0,
            horizon_pitch,
...
fn push_cloud_lens_cyclorama(
    ...
    let alpha = alpha.clamp(0.0, 255.0);
    if alpha <= 0.0 || half_width <= 0.05 || half_thickness <= 0.02 {
        return;
    }
    let yaw0 = center_yaw - half_width;
    let yawm = center_yaw;
    let yaw1 = center_yaw + half_width;
    let pitch0 = center_pitch - slant * 0.5;
    let pitchm = center_pitch;
    let pitch1 = center_pitch + slant * 0.5;
...
fn sky_cloud_wash_color(
    ...
    cyclorama_lerp_rgb(base, tint, (strength * 20.0).clamp(0.0, 255.0) as u8)
}
