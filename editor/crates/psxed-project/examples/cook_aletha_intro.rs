...
fn main() {
    ...
    let base = root.join("source_assets/characters/aletha/Aletha.glb");
    let existing =
        std::fs::read(root.join("assets/models/aletha_delivered/aletha_delivered.psxmdl")).unwrap();
    let old = psx_asset::Model::from_bytes(&existing).unwrap();
    let name = std::env::args()
        .nth(2)
        .unwrap_or_else(|| "aletha_wake_up".into());
    let hz = 12;
    ...
    let package = preview_model_with_animation_sources(
        &base,
        &[root.join(format!("source_assets/animations/player/{name}.glb"))],
        config,
    )
    .unwrap();
    ...
    std::fs::write(
        root.join(format!("assets/animations/aletha_delivered/{name}.psxanim")),
        &clip.bytes,
    )
    .unwrap();
}
