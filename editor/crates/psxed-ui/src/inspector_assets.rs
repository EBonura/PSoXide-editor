...
impl AvailableAnimationClip {
    pub(crate) fn from_stored_path(stored_path: String) -> Self {
        Self {
...
pub(crate) fn available_animation_clips(project_root: &Path) -> Vec<AvailableAnimationClip> {
...
pub(crate) fn collect_available_animation_clips(
    dir: &Path,
    project_root: &Path,
...
pub(crate) fn should_skip_animation_scan_dir(path: &Path) -> bool {
...
pub(crate) fn is_psxanim_path(path: &Path) -> bool {
    path.extension()
        .and_then(|ext| ext.to_str())
        .is_some_and(|ext| ext.eq_ignore_ascii_case("psxanim"))
}

pub(crate) fn project_relative_path_string(path: &Path, project_root: &Path) -> String {
...
pub(crate) fn animation_clip_source_picker(
    ui: &mut egui::Ui,
    id_salt: egui::Id,
...
pub(crate) fn set_model_animation_clip_source(
    clip: &mut psxed_project::ModelAnimationClip,
    new_path: &str,
) -> bool {
...
pub(crate) fn should_auto_rename_animation_clip(name: &str, old_path: &str) -> bool {
...
pub(crate) fn animation_clip_default_name(path: &str) -> String {
    Path::new(path)
        .file_stem()
...
pub(crate) fn animation_clip_label_for_path(path: &str) -> String {
...
pub(crate) fn material_texture_picker(
    ui: &mut egui::Ui,
    current: &mut Option<ResourceId>,
        ...
        egui::popup::popup_below_widget(
            ...
            |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.add_sized(
                        [search_width, 24.0],
                        egui::TextEdit::singleline(search).hint_text("Search textures"),
                    );
                    if ui
                        .add_enabled(!search.is_empty(), egui::Button::new("Clear"))
                        .clicked()
                    {
                        search.clear();
                    }
                ...
                let matches: Vec<&TexturePickerOption> = options
                    .iter()
                    .filter(|option| texture_picker_option_matches(&option.name, &query))
                    .collect();
                ...
                if matches.is_empty() {
                    ...
                    ui.centered_and_justified(|ui| {
                        ui.label(RichText::new("No matching textures").color(STUDIO_TEXT_WEAK));
                    });
                } else {
                    egui::ScrollArea::vertical()
                        .id_salt("material_texture_picker_scroll")
                        .max_height(MATERIAL_TEXTURE_PICKER_POPUP_HEIGHT)
                        .auto_shrink([false, true])
                        .show(ui, |ui| {
                            for option in &matches {
                                if material_texture_picker_row(
                                    ui,
                                    &option.name,
                                    option.thumb,
                                    *current == Some(option.id),
                ...
                if !query.is_empty() {
                    ui.add_space(4.0);
                    ui.label(
                        RichText::new(format!("{} of {} textures", match_count, options.len()))
                            .color(STUDIO_TEXT_WEAK)
                            .small(),
                    );
                }
...
pub(crate) fn texture_picker_option_matches(name: &str, query: &str) -> bool {
    if query.is_empty() {
        return true;
    }
    let name = name.to_ascii_lowercase();
    query.split_whitespace().all(|part| name.contains(part))
}

pub(crate) fn material_texture_picker_row(
    ui: &mut egui::Ui,
    label: &str,
    thumb: Option<(egui::TextureId, PsxtStats)>,
    selected: bool,
    ...
    let (rect, response) = ui.allocate_exact_size(
        Vec2::new(width, MATERIAL_TEXTURE_PICKER_ROW_HEIGHT),
        Sense::click(),
    );
    ...
    if fill != Color32::TRANSPARENT {
        painter.rect_filled(rect.shrink2(Vec2::new(1.0, 2.0)), 4.0, fill);
    }

    let thumb_rect = Rect::from_min_size(
        Pos2::new(
            rect.left() + 5.0,
            rect.center().y - MATERIAL_TEXTURE_PICKER_THUMB_SIZE * 0.5,
        ),
        Vec2::splat(MATERIAL_TEXTURE_PICKER_THUMB_SIZE),
    );
    painter.rect_filled(thumb_rect, 3.0, STUDIO_PANEL);
    match thumb {
        Some((id, stats)) => {
            if stats.index_zero_transparent {
                draw_checker_preview(&painter, thumb_rect, Color32::from_rgb(20, 26, 32));
            }
            painter.image(
                id,
                thumb_rect,
