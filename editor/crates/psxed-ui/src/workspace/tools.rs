...
impl EditorWorkspace {
    ...
    fn brush_drag_brushes(drag: BrushDrag) -> Vec<psxed_project::brush::Brush> {
        ...
        // The shared generator reports why a recipe failed; a live drag has no
        // use for that (the user sees nothing appear and adjusts the box), so
        // the diagnosis is dropped here and consumed by the authoring MCP.
        psxed_project::brush_primitives::generate(min, max, drag.settings, drag.grid_step.max(1))
            .map(|generated| generated.brushes)
            .unwrap_or_default()
    }
