...
fn main() {
    // Throwaway: one strip of frames per clip, so a folder of unnamed takes
    ...
    let world_height: i32 = a.next().unwrap().parse().unwrap();
    let out_dir = a.next().unwrap();
    ...
    for path in &clips {
        ...
        let stem = std::path::Path::new(path)
            ...
            .unwrap()
            .to_string();
        for (step, time) in [0.0f64, 0.17, 0.34, 0.5, 0.67, 0.84].iter().enumerate() {
